...
public class CustomJsonEncoder extends ParsingEncoder implements Parser.ActionHandler {
    ...
    CustomJsonEncoder(Schema sc, JsonGenerator out) throws IOException {
        configure(out);
        this.parser =
     ...
     */
    @Deprecated
    public CustomJsonEncoder configure(JsonGenerator generator) throws IOException {
        if (null == generator)
    ...
    private void writeByteArray(byte[] bytes, int start, int len)
            throws IOException {
        out.writeString(
                new String(bytes, start, len, CustomJsonEncoder.CHARSET));
    }
