...
class MetricsListener extends Actor with ActorLogging {
  ...
  val nodeStateQueryRouter = context.actorOf(FromConfig.props(), "node-state-query-router")
  ...
  def receive = {
    case ClusterMetricsChanged(clusterMetrics) =>
      clusterMetrics.filter(_.address == selfAddress) foreach { nodeMetrics =>
        ...
        nodeStateQueryRouter ! Broadcast(NodeStateChanged(selfAddress.toString, totalCpuNum, maxHeap, freeCpuNum, maxHeap - usedHeap, load))
      }
  }
